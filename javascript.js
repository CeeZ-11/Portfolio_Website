...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let img = imgUrl.src;
    imgUrl.src = 'https://api.screenshotmachine.com?key=ae72d1&url=' + encodeURIComponent(img) + '&dimension=1024x768';
...
function goToNextSlide(){
    const slides = document.querySelectorAll('.carousel-slide');
    currentIndex = (currentIndex + 1) % slides.length;length;
    showSlide(currentIndex);
}
...
function onClickMenu(menu){
    ...
    menu.addEventListener('mouseover', function(){
        if(!menu.classList.contains("change1") && !menu.classList.contains("change2")){
          menu.style.gap = '15%';
